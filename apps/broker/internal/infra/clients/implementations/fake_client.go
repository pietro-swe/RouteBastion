...
func (f *FakeRouteOptimizer) OptimizeSync(_ context.Context, input shared.OptimizationRequestInput) ([]shared.OptimizationRequestOutput, error) {
	if len(input.Vehicles) == 0 {
		return make([]shared.OptimizationRequestOutput, 0), errors.New("no vehicles provided")
	}
	...
	steps := make([]shared.RouteStep, len(stops))
	current := start
	totalDistance := 0.0
	...
	for len(visited) < len(stops) {
		...
		if bestIdx == -1 {
			return make([]shared.OptimizationRequestOutput, 0), errors.New("no valid next stop found — constraint deadlock")
		}
		...
		stepKind := shared.RouteStepKind(stop.Type)
	...
	result := make([]shared.OptimizationRequestOutput, 1)

	result = append(result, shared.OptimizationRequestOutput{
